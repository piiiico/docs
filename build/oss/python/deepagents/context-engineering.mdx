...
You do **not** need middleware when tools alone use context or `runtime.store`; tools receive `runtime.context` and `runtime.store` directly via the `ToolRuntime` parameter. Add middleware only when tools should be packaged with an update to the system prompt.
...
Runtime context **propagates to all subagents**. When a subagent runs, it receives the same context as the parent. See [Subagents](/oss/python/deepagents/subagents#context-management) for per-subagent context (namespaced keys).
