...
When you invoke a parent agent with [runtime context](/oss/python/langchain/runtime), that context automatically propagates to all subagents.
...
class Context:
    user_id: str
    researcher_max_depth: int = 3
    fact_checker_strict_mode: bool = False
...
def flexible_search(query: str, runtime: ToolRuntime[Context]) -> str:
    ...
    max_results = getattr(runtime.context, f"{agent_name}_max_results", 5)
    include_raw = getattr(runtime.context, f"{agent_name}_include_raw", False)
