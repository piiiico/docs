...
You do **not** need middleware when tools alone use context or `runtime.store`; tools receive `runtime.context` and `runtime.store` directly via the `ToolRuntime` parameter. Add middleware only when the system prompt itself must vary per request.
...
Define your context schema as a dataclass (Python) or Zod schema (TypeScript), then pass an instance at invoke time. Tools access context through the `runtime` parameter:
...
Runtime context **propagates to all subagents**. When a subagent runs, it receives the same context as the parent. See [Subagents](/oss/javascript/deepagents/subagents#context-management) for per-subagent context (namespaced keys).
