...
When you invoke a parent agent with [runtime context](/oss/javascript/langchain/runtime), that context automatically propagates to all subagents.
...
const getUserData = tool(
  async ({ query }, runtime: ToolRuntime<unknown, z.infer<typeof contextSchema>>) => {
    const userId = runtime.context.userId;
    return `Data for user ${userId}: ${query}`;
  },
  {
    name: "get_user_data",
...
All subagents receive the same parent context. To pass configuration that is specific to a particular subagent, use **namespaced keys**: prefix context fields with the subagent's name:
...
const contextSchema = z.object({
  userId: z.string(),
  researcherMaxDepth: z.number().default(3),
  factCheckerStrictMode: z.boolean().default(false),
});
...
const result = await agent.invoke(
  { messages: [new HumanMessage("Research this and verify the claims")] },
  {
    context: {
      userId: "user-123",
      researcherMaxDepth: 3,
      factCheckerStrictMode: true,
    },
  },
);
```
...
Then read the relevant fields inside tools:



```typescript
const verifyClaim = tool(
  async ({ claim }, runtime: ToolRuntime<unknown, z.infer<typeof contextSchema>>) => {
    if (runtime.context.factCheckerStrictMode) {
      return strictVerification(claim);
    }
    return basicVerification(claim);
  },
  {
    name: "verify_claim",
...
const sharedLookup = tool(
  async ({ query }, runtime: ToolRuntime) => {
    ...
    if (agentName === "fact-checker") {
      return strictLookup(query);
    }
    return generalLookup(query);
  },
  {
    name: "shared_lookup",
...
const flexibleSearch = tool(
  async ({ query }, runtime: ToolRuntime<unknown, z.infer<typeof contextSchema>>) => {
    ...
    const maxResults = (ctx as any)[`${agentName}MaxResults`] ?? 5;
    const includeRaw = (ctx as any)[`${agentName}IncludeRaw`] ?? false;
    return performSearch(query, { maxResults, includeRaw });
  },
  {
