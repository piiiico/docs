...
- Fixed tracer sessions to use an improved limit+1 approximation.
- Added a radio card component to the frontend for better UI interactions.
- Improved display of tools on the prompt commit page.
- Migrated Save Prompt modal to use the design system Dialog for a consistent look.
- Fixed operator precedence bug in insights leading to incorrect results.
- Added URL allowlist support for LLM Auth Proxy to enhance security.
- Enabled audit logs by default for self-hosted deployments using a go queue.
- Enforced NOT NULL DEFAULT constraint on OAuth token metadata column.
- Integrated PAQ smithdb with smith-go backend for enhanced functionality.
- Enforced MCP server RBAC on the frontend for increased security.
- Added x service authentication support in sandboxes for improved integration.
- Ensured SSRF policy permits localhost for local development environments.
- Enhanced feedback system to ensure updates across all endpoints.
- Fixed memory check issues in hub for local development and authentication.
- Added typed proxy header model for sandboxes with distinct types for enhanced security.
- Improved handling of feedback deltas on the frontend.
- Deleted MCP servers when organization/project changes to maintain system integrity.
