...
1. **Enable LLM auth proxy:** toggle on for your organization.
2. **JWT audience:** the `aud` claim value the proxy will validate (for example, `my-audience`). This must match `jwtAudiences` in the auth proxy chart in [Step 4](#4-install-the-auth-proxy-helm-chart).
...
- **`jwksJson` (inline):** Paste the JWKS JSON directly into `values.yaml`. Simpler for testing or air-gapped environments, but requires a chart update to rotate keys.
...
- **Sidecar:** run the service in the same pod as the proxy. Add the container under `authProxy.deployment.sidecars` and any required volumes under `authProxy.deployment.volumes` in `values.yaml`.
- **Separate deployment:** deploy the service independently and point `extAuthz.serviceUrl` at it.
